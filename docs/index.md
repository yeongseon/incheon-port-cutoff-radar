...
| 서비스 | URL |
|--------|-----|
| 🖥️ 프론트엔드 | [http://localhost:3000](http://localhost:3000) |
| ⚙️ 백엔드 API | [http://localhost:8000](http://localhost:8000) |
...
```
👤 사용자 브라우저
  → 🖥️ 프론트엔드 (React 19 + TypeScript + Tailwind)
    → ⚙️ 백엔드 API (FastAPI + Pydantic v2)
      → 📊 데이터 집계 계층
      → 🧮 리스크 엔진 (규칙 기반 v1.0.0)
      → 🗄️ Redis (캐시) + PostgreSQL (영구 저장)
      → 🌍 외부 공공 API
```
...
| 계층 | 스택 |
|------|------|
| 🖥️ 프론트엔드 | React 19, TypeScript, Vite 8, Tailwind CSS v4, Recharts |
| ⚙️ 백엔드 | Python 3.10+, FastAPI, Pydantic v2, SQLAlchemy |
| 🗄️ 데이터베이스 | PostgreSQL 16, Redis 7 |
| 🐳 인프라 | Docker Compose |
| 📖 문서 | MkDocs Material |
