      ...
      {reasons.map((r) => (
        <div key={r.code} className="bg-slate-50 rounded-lg p-4 border border-slate-200">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-sm text-blue-600 font-semibold">+{r.impact_minutes.toFixed(0)} min</span>
          </div>
          ...
          <span className="text-xs text-slate-400 mt-1 inline-block">{r.contribution_percent}% contribution</span>
        </div>
      ))}
    </div>
  );
